...
- **新赛事**：`scripts/discover-races.ts` 两段式巡检——先联网粗筛未来半年内即将举办的赛事（只要名称/地区/日期，去重在本地做），再对通过去重与日期校验的候选逐场核实组委会官方公告，核实不过直接丢弃（实测一轮：粗筛 30 场 → 核实 8 场 → 入库 4 场，拦下 4 场无官方依据的）
- **复查**：`scripts/update-status.ts` 调用阿里云百炼千问 API（联网搜索），复查范围由 `src/lib/schedule.ts` 的分级调度决定；每次跑完会打印 token 账单，也可用 `--limit=N` 小量试跑
- **成本**：代码托管/部署/调度均使用免费额度；千问 API 只花在“还可能变化”的赛事上，赛期已过（当前 162 场）与报名已尘埃落定的一律不查（实测日均查询 156 场 → 47 场，7 天累计 1092 → 332 次，降幅 70%）；实测单场约 4800 tokens（输入占 75%），每日复查约 22 万 tokens、巡检一轮约 5.6 万
